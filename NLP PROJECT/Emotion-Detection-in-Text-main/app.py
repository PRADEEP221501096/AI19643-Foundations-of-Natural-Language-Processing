...
import pandas as pd
import numpy as np
from assets import nltkdataRun
...
def main():
    ...
    if choice == "Home":
        ...
        st.subheader("Emotion Detectior in TAMIL Text")

        with st.form(key='emotion_clf_form'):
            text = st.text_area("Enter your TAMIL text here:")
            submit_text = st.form_submit_button(label='அனுப்பவும்')

        if submit_text:
            col1, col2 = st.columns(2)

            raw_text = nltkdataRun(text=text)
